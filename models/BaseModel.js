...
class BaseModel {
    ...
    execSql(param, callback) {
        Comm.execSql(config.agent, param, callback);
    }
    execSqlAgent2(param, callback) {
        Comm.execSql(config.agent2, param, callback);
    }
    /**
     * 执行事物sql集合
     * @param sqls
    ...
    _getConnection(callback) {
        Async.auto({
            pool: cb => {
                Comm.getPoolSer(config.agent, cb);
            },
